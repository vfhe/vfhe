...
from .._native_compiler import enable_asm_sources
...
def find_vfhe_root() -> Path:
    ...
    candidates = [Path(__file__).resolve().parents[2] / "_source"]
...
def _assemble_ffi(
    ...
    ffi.set_source(
        ...
        define_macros=[tuple(d) for d in engine["defines"]],
        libraries=["m"],
        extra_objects=[str(archive)],
        # The engine flags must match the archive's: the headers' types
        # depend on them.
        extra_compile_args=["-O3", "-std=gnu11", *engine["flags"], *extra_compile_args],
