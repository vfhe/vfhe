...
The C sources are compiled per engine (meson.build) and ``_vfhe_native``
...
from _vfhe_native import active, ffi, lib, runnable
...
def _warn_if_leaving_performance_on_the_table(engine: str, choices: list[str]) -> None:
    """Hint (once) when a faster engine than the active one could run here —
...
_warn_if_leaving_performance_on_the_table(active, runnable)
