...
Imports nothing from vfhe: the package's own __init__ chain reaches here, so
anything it took from vfhe would be circular. `_vfhe_cpu` and `_vfhe_engines`
are top-level, built by meson, and absent until then.
"""
...
from _vfhe_cpu import lib as _cpu  # pyright: ignore[reportMissingImports]
from _vfhe_engines import ENGINES  # pyright: ignore[reportMissingImports]
...
# Names an installed engine could take here, best first: vfhe.engine's
