...
def add_c_file(path: str):
    """Add a C file to be compiled with the library."""
    abs_path = os.path.abspath(path)
    if not os.path.exists(abs_path):
        raise FileNotFoundError(f"Source file not found: {abs_path}")
    if not abs_path.endswith(".c"):
        raise ValueError(f"Unsupported file type (expected .c): {abs_path}")
    if abs_path not in c_files:
        c_files.append(abs_path)
        logger.info("Added source file: %s", abs_path)
...
def add_cdef_file(path: str):
    ...
    if abs_path not in cdef_files:
        ...
        logger.info("Added CFFI declaration file: %s", abs_path)
    ...
    for root, _, files in os.walk(abs_dir):
        for file in files:
            ...
            if file.endswith(".c"):
                ...
                added_any = True
    if not added_any:
        logger.warning("No custom files (.c, .cdef) found in directory: %s", abs_dir)
...
def clear_extensions():
    ...
    for path in _temp_c_files:
        try:
            ...
            logger.warning("Failed to delete temporary C file %s: %s", path, e)
    _temp_c_files.clear()
