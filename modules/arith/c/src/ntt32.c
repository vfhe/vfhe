...
static inline void FwdButterfly32(__m512i *X, __m512i *Y, __m512i W, __m512i W_precon,
...
static inline void InvButterfly32(__m512i *X, __m512i *Y, __m512i W, __m512i W_precon,
                                  __m512i neg_modulus, __m512i twice_modulus)
{
...
static inline void InvButterfly_Final_32(__m512i *X, __m512i *Y, __m512i v_inv_n,
...
static uint64_t barrett_factor_32(uint64_t val, uint64_t q)
{
    unsigned __int128 num = (unsigned __int128)val << 32;
                                  ...
                                  size_t level, size_t offset_i, NTT_Plan plan)
{
        ...
        if (sub_n >= 16)
        {
            ...
            { // t = 4
                ...
                for (size_t i = 0; i < sub_n / 16; i++)
                {
                    ...
                    FwdButterfly32(&x[slice], &x[slice + 1], w_i, wp_i, minus_q, q2);
                }
            }
            l++;
    ...
    else
    {
        ...
        ntt_CT_NR_internal_32(x, ws, w_precon, sub_n / 2, level + 1, 2 * offset_i, plan);
        ntt_CT_NR_internal_32(x + t, ws, w_precon, sub_n / 2, level + 1, 2 * offset_i + 1, plan);
    }
}
                                  ...
                                  NTT_Plan plan)
{
    ...
    if (sub_n <= 1024)
    {
        ...
        if (sub_n >= 16)
        {
            ...
            { // t = 1
                ...
                for (size_t i = 0; i < sub_n / 16; i++)
                {
                    ...
                    InvButterfly32(&x[slice], &x[slice + 1], w_i, wp_i, minus_q, q2);
                }
            }
            l++;
            ...
            { // t = 2
                ...
                for (size_t i = 0; i < sub_n / 16; i++)
                {
                    ...
                    InvButterfly32(&v_X, &v_Y, w_i, wp_i, minus_q, q2);
                    x[slice] = v_X;
                    x[slice + 1] = v_Y;
                }
            }
        ...
        for (; (1ULL << (l + is_top_level)) < sub_n; l++)
        {
            ...
            for (size_t i = 0; i < m_sub; i++)
            {
                ...
                for (size_t j = slice; j < slice + t; j++)
                {
                    InvButterfly32(&x[j], &x[j + t], w_i, wp_i, minus_q, q2);
                }
            }
        }
        ...
        if (is_top_level)
        {
            ...
            uint64_t inv_n_w_prime = barrett_factor_32(inv_n_w, q);
            __m512i v_inv_n_w = _mm512_set1_epi64(inv_n_w);
            __m512i v_inv_n_w_prime = _mm512_set1_epi64(inv_n_w_prime);
            for (size_t j = 0; j < t; j++)
            {
                InvButterfly_Final_32(&x[j], &x[j + t], v_inv_n, v_inv_n_prime, v_inv_n_w,
    ...
    {
        size_t t = sub_n >> 4;
        ntt_GS_RN_internal_32(x, ws, w_precon, sub_n / 2, l_merge - 1, 2 * offset_i, 0, inv_n,
                              plan);
        ntt_GS_RN_internal_32(x + t, ws, w_precon, sub_n / 2, l_merge - 1, 2 * offset_i + 1, 0,
                              inv_n, plan);
        if (is_top_level)
        {
            uint64_t inv_n_prime = barrett_factor_32(inv_n, q);
