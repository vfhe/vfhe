...
static inline void FwdButterfly64(__m512i *X, __m512i *Y, __m512i W, __m512i W_precon,
                                  __m512i neg_modulus, __m512i twice_modulus)
{
    *X = _mm512_min_epu64(*X, _mm512_sub_epi64(*X, twice_modulus));
    __m512i Q = mulhi_approx_64(W_precon, *Y);
...
static inline void InvButterfly64(__m512i *X, __m512i *Y, __m512i W, __m512i W_precon,
                                  __m512i neg_modulus, __m512i twice_modulus)
{
    ...
    *X = _mm512_mask_add_epi64(*X, sign_bits, *X, twice_modulus);
    ...
    __m512i Q = mulhi_approx_64(W_precon, T);
...
static inline void InvButterfly_Final_64(__m512i *X, __m512i *Y, __m512i v_inv_n,
                                         ...
                                         __m512i q_vec)
{
    ...
    __m512i Q1 = mulhi_64(v_inv_n_prime, X_plus_Y);
    ...
    *X = _mm512_add_epi64(prod_X, Q1_p);
    ...
    __m512i Q2 = mulhi_64(v_inv_n_w_prime, T);
...
static uint64_t barrett_factor_64(uint64_t val, uint64_t q)
{
    unsigned __int128 num = (unsigned __int128)val << 64;
    return (uint64_t)(num / q);
}

// Reuse layout helper functions
                                  ...
                                  uint64_t q, size_t level, size_t offset_i, NTT_proc proc)
{
    ...
    else
    {
        ...
        ntt_CT_NR_internal_64(x, ws, w_precon, sub_n / 2, q, level + 1, 2 * offset_i, proc);
        ntt_CT_NR_internal_64(x + t, ws, w_precon, sub_n / 2, q, level + 1, 2 * offset_i + 1, proc);
    }
}
...
void ntt_forward_64(uint64_t *out, uint64_t *in, NTT_proc proc)
{
    ...
    ntt_CT_NR_internal_64((__m512i *)out, (__m512i **)proc->ws_fwd, (__m512i **)proc->w_precon_fwd,
                                  ...
                                  uint64_t inv_n, NTT_proc proc)
{
    ...
    if (sub_n <= 1024)
    {
        ...
        if (sub_n >= 16)
        {
            ...
            { // t = 1
                ...
                for (size_t i = 0; i < sub_n / 16; i++)
                {
                    ...
                    InvButterfly64(&x[slice], &x[slice + 1], w_i, wp_i, minus_q, q2);
                }
            }
            l++;
            ...
            { // t = 2
                ...
                for (size_t i = 0; i < sub_n / 16; i++)
                {
                    ...
                    InvButterfly64(&v_X, &v_Y, w_i, wp_i, minus_q, q2);
                    x[slice] = v_X;
                    x[slice + 1] = v_Y;
                }
            }
        ...
        if (is_top_level)
        {
            ...
            for (size_t j = 0; j < t; j++)
            {
                InvButterfly_Final_64(&x[j], &x[j + t], v_inv_n, v_inv_n_prime, v_inv_n_w,
...
void ntt_reverse_64(uint64_t *out, uint64_t *in, NTT_proc proc)
{
    ...
    ntt_GS_RN_internal_64((__m512i *)out, (__m512i **)proc->ws_inv, (__m512i **)proc->w_precon_inv,
