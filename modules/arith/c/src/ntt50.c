                                  ...
                                  size_t level, size_t offset_i, NTT_Plan plan)
{
    ...
    else
    {
        ...
        ntt_CT_NR_internal_50(x, ws, w_precon, sub_n / 2, level + 1, 2 * offset_i, plan);
        ntt_CT_NR_internal_50(x + t, ws, w_precon, sub_n / 2, level + 1, 2 * offset_i + 1, plan);
    }
}
...
void ntt_forward_50(uint64_t *out, uint64_t *in, NTT_Plan plan)
{
    ...
    ntt_CT_NR_internal_50((__m512i *)out, (__m512i **)plan->ws_fwd, (__m512i **)plan->w_precon_fwd,
                          plan->n, 0, 0, plan);
                                  ...
                                  NTT_Plan plan)
{
    ...
    {
        size_t t = sub_n >> 4;
        ntt_GS_RN_internal_50(x, ws, w_precon, sub_n / 2, l_merge - 1, 2 * offset_i, 0, inv_n,
                              plan);
        ntt_GS_RN_internal_50(x + t, ws, w_precon, sub_n / 2, l_merge - 1, 2 * offset_i + 1, 0,
                              inv_n, plan);
        if (is_top_level)
        {
            uint64_t inv_n_prime = barrett_factor_52(inv_n, q);
...
void ntt_reverse_50(uint64_t *out, uint64_t *in, NTT_Plan plan)
{
    ...
    ntt_GS_RN_internal_50((__m512i *)out, (__m512i **)plan->ws_inv, (__m512i **)plan->w_precon_inv,
